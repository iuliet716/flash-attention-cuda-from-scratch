...
// Shared memory has 32 banks of 4 bytes. In the QK^T loop of step 05,
// lane `i` reads K row `i`:
//
//   Ktile[lane * d + k]  ->  bank (lane * d + k) % 32
//
// Since d is a multiple of 32, `lane * d` contributes nothing mod 32:
// all 32 lanes hit the bank of column k at the same time. Every read of
// the dot-product loop is serialized by the hardware (bank conflict).
//
// XOR swizzling permutes the float4 chunks WITHIN each row when the tile
// is stored, using the row index as the key:
//
//   physical chunk = logical chunk ^ (row % 8)
//
// Reads use the same formula, so no extra memory is needed (unlike
// padding). Within each 8-lane phase of a float4 access, the 8 rows have
// 8 different keys, so the lanes land on 8 different chunk slots
// (32 different banks) and the access is conflict-free.
//
// The XOR only shuffles chunks inside aligned groups of 8, so it stays in
// bounds as long as d/4 is a multiple of 8 (d % 32 == 0).
...
__global__ void fused_attention_kernel(
    ...
    float scale)
{
    // Only the K tile is swizzled: it is the only tile read with a
    // lane-indexed row.
    // - Q reads (Qtile[warp * d + k]) are warp-uniform -> broadcast, no conflict
    // - V reads (Vtile[c * d + lane + 32 * i]) are consecutive across lanes
    //   -> one word per bank, no conflict
