    ...
    float scale)
{
    ...
    const int dw = d / WARPS;   // O columns per warp
    ...
    for (int tile = 0; tile < N; tile += BC) {
        ...
        // warp 0 assigns one lane to each query row.
        if (warp == 0 && lane < BR) {
            const int r = lane;
            ...
            // exponentials and their sum for this tile
            float l_tile = 0.0f;
            for (int c = 0; c < BC; ++c) {
                const float p =
            ...
            // merge with the running softmax state
