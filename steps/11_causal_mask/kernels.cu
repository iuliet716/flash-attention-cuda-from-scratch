...
// ---- the m16n8k16 tensor-core primitive (same as step 10) ---------------
//
...
__global__ void fused_attention_kernel(
    ...
    float scale)
{
    // Step 10 kernel + causal masking as WORK SKIPPING.
    //
    // Causal attention (row r attends to cols c <= r) masks ~half of the
    // score matrix. A naive implementation computes everything and then
    // overwrites with -inf, paying full price for discarded work. The
    // tiled structure instead classifies each KV tile per Q block:
    //
    //   tile end <= q_base            : fully visible -> unmasked fast path
    //   tile crosses the diagonal     : per-element mask (one tile: BR==BC)
    //   tile start >  last Q row      : fully masked  -> NEVER LOOPED OVER
    //
    // The third class is the win: the KV loop runs to
    // min(N, q_base + BR) instead of N, so block bx executes bx+1 tiles
    // and total work halves. Every prior step made the same work faster;
    // this one removes the work itself.
    //
    // The per-element mask relies on the mma layout knowledge from step
    // 10: thread (g, tig) knows its s[j] elements sit at rows r_lo / r_hi
    // and column tile + 8j + 2*tig + {0,1}, so visibility is one integer
    // compare. IS_CAUSAL is a template parameter: the non-causal
    // instantiation compiles the compares away entirely.
    //
    // Blocks now have unequal work (block 0: one tile, last block: N/BC
    // tiles). With thousands of blocks the SM scheduler absorbs the
    // imbalance by backfilling short blocks.
    ...
    // kernel. Rows past N load zeros so the padding scores stay finite.
    uint32_t qa[KK][4];
#pragma unroll
...
#pragma unroll
    ...
    // work skipping: causal blocks stop at their own diagonal
    const int kv_limit = IS_CAUSAL ? min(N, q_base + BR) : N;
    const int n_tiles = (kv_limit + BC - 1) / BC;
...
#pragma unroll
        ...
        // visibility: col < N always; col <= row when causal. Rows r_lo
        // and r_hi see different columns, so the predicates split. For
        // tiles left of the diagonal (col <= q_base <= row) the causal
        // compare is always true; only the diagonal tile actually masks.
        float mt[2] = {-FLT_MAX, -FLT_MAX};
#pragma unroll
        for (int j = 0; j < SB; ++j) {
#pragma unroll
            for (int e = 0; e < 2; ++e) {
                const int col = tile + 8 * j + 2 * tig + e;
                if (col < N && (!IS_CAUSAL || col <= r_lo))
                    mt[0] = fmaxf(mt[0], s[j][e]);
                if (col < N && (!IS_CAUSAL || col <= r_hi))
...
#pragma unroll
            for (int e = 0; e < 2; ++e) {
                const int col = tile + 8 * j + 2 * tig + e;
                const bool in0 = col < N && (!IS_CAUSAL || col <= r_lo);
                const bool in1 = col < N && (!IS_CAUSAL || col <= r_hi);
                const float p0 = in0 ? __expf(scale * (s[j][e] - m[0])) : 0.0f;
                const float p1 = in1 ? __expf(scale * (s[j][2 + e] - m[1])) : 0.0f;
                s[j][e] = p0;
                s[j][2 + e] = p1;
                lt[0] += p0;
                lt[1] += p1;
    ...
    int N, float scale, int batch_count, cudaStream_t stream)
{
    ...
    if (smem_bytes > 48 * 1024) {
        ...
        cudaFuncSetAttribute(fused_attention_kernel<D, IS_CAUSAL>,
                             cudaFuncAttributeMaxDynamicSharedMemorySize,
                             (int)smem_bytes);
    }
    fused_attention_kernel<D, IS_CAUSAL><<<grid, threads, smem_bytes, stream>>>(
        dQ, dK, dV, dO, N, scale);
}

void launch_fused_attention(
    const __half* dQ, const __half* dK, const __half* dV, __half* dO,
    int N, int d, float scale, bool is_causal, int batch_count, cudaStream_t stream)
{
    if (d == 64) {
        if (is_causal) launch_impl<64, true>(dQ, dK, dV, dO, N, scale, batch_count, stream);
        else           launch_impl<64, false>(dQ, dK, dV, dO, N, scale, batch_count, stream);
    } else if (d == 128) {
        if (is_causal) launch_impl<128, true>(dQ, dK, dV, dO, N, scale, batch_count, stream);
        else           launch_impl<128, false>(dQ, dK, dV, dO, N, scale, batch_count, stream);
    }
